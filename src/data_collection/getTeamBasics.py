...
def getPlayoffPicture():
	...
	del eastPlayoffTeams[0]
	del eastPlayoffTable[7]
	del eastPlayoffTable[8]
	del eastPlayoffTable[9]
	del eastPlayoffTable[10]
	del eastPlayoffTable[11]
	westPlayoffTeams = response.json()['resultSets'][1]['rowSet']
	westPlayoffTable = pd.DataFrame(eastPlayoffTeams)
	del westPlayoffTeams[0]
	del westPlayoffTable[7]
	del westPlayoffTable[8]
	del westPlayoffTable[9]
	del westPlayoffTable[10]
	del westPlayoffTable[11]

	return eastPlayoffTable, westPlayoffTable
...
database = create_engine('sqlite:///csvDB.db')
...
for i in range(0, 4):
	...
	# westgame.to_sql('teamStats', database, if_exists='append', index = False)
	westPlayers.to_csv('WestPlayers' + str(i) + '.csv')
	# westPlayers.to_sql('playerStats', database, if_exists='append', index = False)

	eastgame.to_csv('EastGame' + str(i) + '.csv')
	# eastgame.to_sql('teamStats', database, if_exists='append', index = False)

	eastPlayers.to_csv('EastPlayers' + str(i) + '.csv')
	# eastPlayers.to_sql('playerStats', database, if_exists='append', index = False)


player_db = create_engine('sqlite:///seasonstats.sqlite')
teamPlayers = players2015
teamLog = gameids2015
teamPlayerID = teamPlayers['PERSON_ID']
teamPlayerGameLog = []
for personID in teamPlayerID:
	teamPlayerGameLog.append(pd.DataFrame(gb.player.game_logs(personID).logs()))

teamPlayerGameLog = pd.concat(teamPlayerGameLog) 
teamPlayerGameLog = pd.merge(teamPlayerGameLog, teamPlayers, right_on = ['PERSON_ID'], left_on = ['Player_ID'], how = 'outer')
teamPlayerGameLog.to_csv('AllGames.csv')
teamPlayerGameLog.to_sql('playerstats', player_db, if_exists='replace', index=False)
